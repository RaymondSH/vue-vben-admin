...
<script lang="ts" setup>
  ...
  import { listAllDepart, deleteDeparts } from '/@/api/system/depart';
  import DepartFormModal from '/@/views/system/dept/DepartFormModal.vue';
  ...
  async function loadRootTreeData() {
    try {
      loading.value = true;
      treeData.value = [];
      const result = await listAllDepart();
      console.log(result);
      ...
      if (expandedKeys.value.length === 0) {
        ...
        if (selectedKeys.value.length === 0) {
          ...
          console.log(currentDepart.value);
  ...
  async function doDeleteDepart(idListRef, confirm = true) {
    ...
    if (idList.length > 0) {
      try {
        loading.value = true;
        await deleteDeparts({ ids: idList.join(',') }, confirm);
